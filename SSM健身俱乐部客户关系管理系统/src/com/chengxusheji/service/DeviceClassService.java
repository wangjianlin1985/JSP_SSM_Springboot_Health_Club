...
public class DeviceClassService {
    ...
    /*根据主键获取设备类别记录*/
    public DeviceClass getDeviceClass(int classId) throws Exception  {
        DeviceClass deviceClass = deviceClassMapper.getDeviceClass(classId);
        return deviceClass;
    }
    ...
    /*删除一条设备类别记录*/
    public void deleteDeviceClass (int classId) throws Exception {
        deviceClassMapper.deleteDeviceClass(classId);
    }

    /*删除多条设备类别信息*/
    public int deleteDeviceClasss (String classIds) throws Exception {
    	String _classIds[] = classIds.split(",");
    	for(String _classId: _classIds) {
    		deviceClassMapper.deleteDeviceClass(Integer.parseInt(_classId));
    	}
