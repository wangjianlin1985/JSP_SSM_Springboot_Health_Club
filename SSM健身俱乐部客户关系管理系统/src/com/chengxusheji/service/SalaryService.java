...
public class SalaryService {
    ...
    /*按照查询条件分页查询员工工资记录*/
    public ArrayList<Salary> querySalary(String year,String month,Employee employeeObj,String getDate,int currentPage) throws Exception { 
    	...
    	if(!getDate.equals("")) where = where + " and t_salary.getDate like '%" + getDate + "%'";
    	int startIndex = (currentPage-1) * this.rows;
    	return salaryMapper.querySalary(where, startIndex, this.rows);
    }

    /*按照查询条件查询所有记录*/
    public ArrayList<Salary> querySalary(String year,String month,Employee employeeObj,String getDate) throws Exception  { 
     	String where = "where 1=1";
    	if(!year.equals("")) where = where + " and t_salary.year like '%" + year + "%'";
    	if(!month.equals("")) where = where + " and t_salary.month like '%" + month + "%'";
    	if(null != employeeObj &&  employeeObj.getEmployeeNo() != null && !employeeObj.getEmployeeNo().equals(""))  where += " and t_salary.employeeObj='" + employeeObj.getEmployeeNo() + "'";
    ...
    /*当前查询条件下计算总的页数和记录数*/
    public void queryTotalPageAndRecordNumber(String year,String month,Employee employeeObj,String getDate) throws Exception {
     	String where = "where 1=1";
    	if(!year.equals("")) where = where + " and t_salary.year like '%" + year + "%'";
    	if(!month.equals("")) where = where + " and t_salary.month like '%" + month + "%'";
    	if(null != employeeObj &&  employeeObj.getEmployeeNo() != null && !employeeObj.getEmployeeNo().equals(""))  where += " and t_salary.employeeObj='" + employeeObj.getEmployeeNo() + "'";
