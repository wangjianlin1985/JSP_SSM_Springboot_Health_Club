...
public class SalaryController extends BaseController {
	...
	/*ajax方式按照查询条件分页查询员工工资信息*/
	@RequestMapping(value = { "/list" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void list(String year,String month,@ModelAttribute("employeeObj") Employee employeeObj,String getDate,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (year == null) year = "";
		if (month == null) month = "";
		if (getDate == null) getDate = "";
		if(rows != 0)salaryService.setRows(rows);
		List<Salary> salaryList = salaryService.querySalary(year, month, employeeObj, getDate, page);
	    /*计算总的页数和总的记录数*/
	    salaryService.queryTotalPageAndRecordNumber(year, month, employeeObj, getDate);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String year,String month,@ModelAttribute("employeeObj") Employee employeeObj,String getDate,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (year == null) year = "";
		if (month == null) month = "";
		if (getDate == null) getDate = "";
		List<Salary> salaryList = salaryService.querySalary(year, month, employeeObj, getDate, currentPage);
	    /*计算总的页数和总的记录数*/
	    salaryService.queryTotalPageAndRecordNumber(year, month, employeeObj, getDate);
	    /*获取到总的页码数目*/
	    int totalPage = salaryService.getTotalPage();
	    ...
	    request.setAttribute("getDate", getDate);
	    List<Employee> employeeList = employeeService.queryAllEmployee();
	    request.setAttribute("employeeList", employeeList);
	...
	public String delete(@PathVariable Integer salaryId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  salaryService.deleteSalary(salaryId);
	            request.setAttribute("message", "员工工资删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "员工工资删除失败!");
				return "error";
	...
	/*按照查询条件导出员工工资信息到Excel*/
	@RequestMapping(value = { "/OutToExcel" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void OutToExcel(String year,String month,@ModelAttribute("employeeObj") Employee employeeObj,String getDate, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(year == null) year = "";
        if(month == null) month = "";
        if(getDate == null) getDate = "";
        List<Salary> salaryList = salaryService.querySalary(year,month,employeeObj,getDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Salary信息记录"; 
        String[] headers = { "工资id","工资年份","工资月份","工资金额","发放员工","发放日期"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<salaryList.size();i++) {
        	Salary salary = salaryList.get(i); 
        	dataset.add(new String[]{salary.getSalaryId() + "",salary.getYear(),salary.getMonth(),salary.getSalaryMoney() + "",salary.getEmployeeObj().getName(),salary.getGetDate()});
        }
        /*
