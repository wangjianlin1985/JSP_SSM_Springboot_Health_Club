...
public class DepartmentController extends BaseController {
	...
	public void list(String departmentNo,String departmentName,String bornDate,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (departmentNo == null) departmentNo = "";
		if (departmentName == null) departmentName = "";
		if (bornDate == null) bornDate = "";
		if(rows != 0)departmentService.setRows(rows);
		List<Department> departmentList = departmentService.queryDepartment(departmentNo, departmentName, bornDate, page);
	    /*计算总的页数和总的记录数*/
	    departmentService.queryTotalPageAndRecordNumber(departmentNo, departmentName, bornDate);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String departmentNo,String departmentName,String bornDate,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (departmentNo == null) departmentNo = "";
		if (departmentName == null) departmentName = "";
		if (bornDate == null) bornDate = "";
		List<Department> departmentList = departmentService.queryDepartment(departmentNo, departmentName, bornDate, currentPage);
	    /*计算总的页数和总的记录数*/
	    departmentService.queryTotalPageAndRecordNumber(departmentNo, departmentName, bornDate);
	    /*获取到总的页码数目*/
	    int totalPage = departmentService.getTotalPage();
	...
	public void OutToExcel(String departmentNo,String departmentName,String bornDate, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(departmentNo == null) departmentNo = "";
        if(departmentName == null) departmentName = "";
        if(bornDate == null) bornDate = "";
        List<Department> departmentList = departmentService.queryDepartment(departmentNo,departmentName,bornDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Department信息记录"; 
        String[] headers = { "部门编号","部门名称","成立日期"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<departmentList.size();i++) {
        	Department department = departmentList.get(i); 
        	dataset.add(new String[]{department.getDepartmentNo(),department.getDepartmentName(),department.getBornDate()});
        }
        /*
