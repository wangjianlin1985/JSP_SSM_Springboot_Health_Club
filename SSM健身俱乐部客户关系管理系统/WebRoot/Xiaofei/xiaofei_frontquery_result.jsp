...
<%
    ...
    String xiaofeiName = (String)request.getAttribute("xiaofeiName"); //消费项目查询关键字
    UserInfo memberObj = (UserInfo)request.getAttribute("memberObj");
    String xiaofeiTime = (String)request.getAttribute("xiaofeiTime"); //消费时间查询关键字
    String dqsj = (String)request.getAttribute("dqsj"); //到期时间查询关键字
%>
<!DOCTYPE html>
<html>
<head>
		...
		<div class="col-md-9 wow fadeInDown" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	...
			    	<li role="presentation" class="active"><a href="#xiaofeiListPanel" aria-controls="xiaofeiListPanel" role="tab" data-toggle="tab">会员消费列表</a></li>
			    	<li role="presentation" ><a href="<%=basePath %>Xiaofei/xiaofei_frontAdd.jsp" style="display:none;">添加会员消费</a></li>
				</ul>
			  	<!-- Tab panes -->
				    		...
				    		<div class="row">
				    			<div class="col-md-12 top5">
				    				...
				    				<table class="table table-condensed table-hover">
				    					<tr class="success bold"><td>序号</td><td>消费id</td><td>消费项目</td><td>消费金额</td><td>消费会员</td><td>消费时间</td><td>到期时间</td><td>操作</td></tr>
				    					<% 
				    						/*计算起始序号*/
				    	            		...
				    	            		for(int i=0;i<xiaofeiList.size();i++) {
					    	            		...
					    	            		Xiaofei xiaofei = xiaofeiList.get(i); //获取到会员消费对象
 										%>
 										<tr>
 											<td><%=currentIndex %></td>
 											<td><%=xiaofei.getXiaofeiId() %></td>
 											<td><%=xiaofei.getXiaofeiName() %></td>
 											<td><%=xiaofei.getXiaofeiMoney() %></td>
 											<td><%=xiaofei.getMemberObj().getName() %></td>
 											<td><%=xiaofei.getXiaofeiTime() %></td>
 											<td><%=xiaofei.getDqsj() %></td>
 											<td>
	...
	<div class="col-md-3 wow fadeInRight">
		...
		<form name="xiaofeiQueryForm" id="xiaofeiQueryForm" action="<%=basePath %>Xiaofei/frontlist" class="mar_t15" method="post">
			<div class="form-group">
				<label for="xiaofeiName">消费项目:</label>
				<input type="text" id="xiaofeiName" name="xiaofeiName" value="<%=xiaofeiName %>" class="form-control" placeholder="请输入消费项目">
			</div>
            ...
            <div class="form-group">
            	<label for="memberObj_user_name">消费会员：</label>
                <select id="memberObj_user_name" name="memberObj.user_name" class="form-control">
                	<option value="">不限制</option>
	 				<%
	 				for(UserInfo userInfoTemp:userInfoList) {
	 					String selected = "";
 					if(memberObj!=null && memberObj.getUser_name()!=null && memberObj.getUser_name().equals(userInfoTemp.getUser_name()))
 						selected = "selected";
	 				%>
			...
			<div class="form-group">
				<label for="dqsj">到期时间:</label>
				<input type="text" id="dqsj" name="dqsj" value="<%=dqsj %>" class="form-control" placeholder="请输入到期时间">
			</div>
  ...
  <div class="modal-dialog" style="width:900px;" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title"><i class="fa fa-edit"></i>&nbsp;会员消费信息编辑</h4>
      </div>
      <div class="modal-body" style="height:450px; overflow: scroll;">
		  ...
		  <div class="form-group">
		  	 <label for="xiaofei_xiaofeiName_edit" class="col-md-3 text-right">消费项目:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="xiaofei_xiaofeiName_edit" name="xiaofei.xiaofeiName" class="form-control" placeholder="请输入消费项目">
			 </div>
		  </div>
		  <div class="form-group">
		  	 <label for="xiaofei_xiaofeiDesc_edit" class="col-md-3 text-right">消费详情:</label>
		  	 <div class="col-md-9">
			 	<textarea name="xiaofei.xiaofeiDesc" id="xiaofei_xiaofeiDesc_edit" style="width:100%;height:500px;"></textarea>
			 </div>
		  </div>
		  <div class="form-group">
		  ...
		  <div class="form-group">
		  	 <label for="xiaofei_memberObj_user_name_edit" class="col-md-3 text-right">消费会员:</label>
		  	 <div class="col-md-9">
			    <select id="xiaofei_memberObj_user_name_edit" name="xiaofei.memberObj.user_name" class="form-control">
			    </select>
		  	 </div>
		  </div>
		  <div class="form-group">
		  ...
		  <div class="form-group">
		  	 <label for="xiaofei_dqsj_edit" class="col-md-3 text-right">到期时间:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="xiaofei_dqsj_edit" name="xiaofei.dqsj" class="form-control" placeholder="请输入到期时间">
			 </div>
		  </div>
		  <div class="form-group">
...
<script>
//实例化编辑器
var xiaofei_xiaofeiDesc_edit = UE.getEditor('xiaofei_xiaofeiDesc_edit'); //消费详情编辑器
	...
	$.ajax({
		...
		success : function (xiaofei, response, status) {
			if (xiaofei) {
				...
				$("#xiaofei_xiaofeiName_edit").val(xiaofei.xiaofeiName);
				xiaofei_xiaofeiDesc_edit.setContent(xiaofei.xiaofeiDesc, false);
				$("#xiaofei_xiaofeiMoney_edit").val(xiaofei.xiaofeiMoney);
				$.ajax({
					url: basePath + "UserInfo/listAll",
					type: "get",
					success: function(userInfos,response,status) { 
						$("#xiaofei_memberObj_user_name_edit").empty();
						var html="";
		        		$(userInfos).each(function(i,userInfo){
		        			html += "<option value='" + userInfo.user_name + "'>" + userInfo.name + "</option>";
		        		});
		        		$("#xiaofei_memberObj_user_name_edit").html(html);
		        		$("#xiaofei_memberObj_user_name_edit").val(xiaofei.memberObjPri);
					}
				});
				$("#xiaofei_xiaofeiTime_edit").val(xiaofei.xiaofeiTime);
				$("#xiaofei_dqsj_edit").val(xiaofei.dqsj);
