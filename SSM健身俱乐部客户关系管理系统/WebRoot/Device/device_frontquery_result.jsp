...
<%
    ...
    String devicePlace = (String)request.getAttribute("devicePlace"); //设备位置查询关键字
...
<jsp:include page="../header.jsp"></jsp:include>
	<div class="col-md-9 wow fadeInLeft">
		<ul class="breadcrumb">
  			...
  			<li><a href="<%=basePath %>Device/frontlist">设备信息列表</a></li>
  			<li class="active">查询结果显示</li>
  			<a class="pull-right" href="<%=basePath %>Device/device_frontAdd.jsp" style="display:none;">添加设备</a>
		</ul>
		<div class="row">
			<%
				/*计算起始序号*/
			...
			<div class="col-md-3 bottom15" <%=clearLeft %>>
			  <a  href="<%=basePath  %>Device/<%=device.getDeviceId() %>/frontshow"><img class="img-responsive" src="<%=basePath%><%=device.getDevicePhoto()%>" /></a>
			     <div class="showFields">
			     	...
			     	<div class="field">
	            		设备类别:<%=device.getDeviceClassObj().getClassName() %>
			     	</div>
			     	<div class="field">
	            		设备名称:<%=device.getDeviceName() %>
			     	</div>
			     	<div class="field">
	            		设备单价:<%=device.getPrice() %>
			     	</div>
			     	<div class="field">
	            		设备库存:<%=device.getDeviceCount() %>
			     	</div>
			     	<div class="field">
	            		设备位置:<%=device.getDevicePlace() %>
			     	</div>
			     	<div class="field">
	...
	<div class="col-md-3 wow fadeInRight">
		...
		<form name="deviceQueryForm" id="deviceQueryForm" action="<%=basePath %>Device/frontlist" class="mar_t15" method="post">
            <div class="form-group">
            	<label for="deviceClassObj_classId">设备类别：</label>
                <select id="deviceClassObj_classId" name="deviceClassObj.classId" class="form-control">
                	<option value="0">不限制</option>
	 				<%
	 				for(DeviceClass deviceClassTemp:deviceClassList) {
	 					String selected = "";
 					if(deviceClassObj!=null && deviceClassObj.getClassId()!=null && deviceClassObj.getClassId().intValue()==deviceClassTemp.getClassId().intValue())
 						selected = "selected";
	 				%>
 				 <option value="<%=deviceClassTemp.getClassId() %>" <%=selected %>><%=deviceClassTemp.getClassName() %></option>
	 				<%
	 				}
	 				%>
 			</select>
			...
			<div class="form-group">
				<label for="devicePlace">设备位置:</label>
				<input type="text" id="devicePlace" name="devicePlace" value="<%=devicePlace %>" class="form-control" placeholder="请输入设备位置">
			</div>
			<div class="form-group">
  ...
  <div class="modal-dialog" style="width:900px;" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" style="height:450px; overflow: scroll;">
		  ...
		  <div class="form-group">
			 <label for="device_deviceId_edit" class="col-md-3 text-right">设备id:</label>
			 <div class="col-md-9"> 
			 	<input type="text" id="device_deviceId_edit" name="device.deviceId" class="form-control" placeholder="请输入设备id" readOnly>
			 </div>
		  </div> 
		  <div class="form-group">
		  	 <label for="device_deviceClassObj_classId_edit" class="col-md-3 text-right">设备类别:</label>
		  	 <div class="col-md-9">
			    <select id="device_deviceClassObj_classId_edit" name="device.deviceClassObj.classId" class="form-control">
			    </select>
		  	 </div>
		  </div>
		  <div class="form-group">
		  ...
		  <div class="form-group">
		  	 <label for="device_price_edit" class="col-md-3 text-right">设备单价:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="device_price_edit" name="device.price" class="form-control" placeholder="请输入设备单价">
			 </div>
		  </div>
		  <div class="form-group">
		  ...
		  <div class="form-group">
		  	 <label for="device_devicePlace_edit" class="col-md-3 text-right">设备位置:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="device_devicePlace_edit" name="device.devicePlace" class="form-control" placeholder="请输入设备位置">
			 </div>
		  </div>
		  <div class="form-group">
			...
			if (device) {
				...
				$.ajax({
					...
					success: function(deviceClasss,response,status) { 
						$("#device_deviceClassObj_classId_edit").empty();
						var html="";
		        		$(deviceClasss).each(function(i,deviceClass){
		        			html += "<option value='" + deviceClass.classId + "'>" + deviceClass.className + "</option>";
		        		});
		        		$("#device_deviceClassObj_classId_edit").html(html);
		        		$("#device_deviceClassObj_classId_edit").val(device.deviceClassObjPri);
					}
				});
